<script lang="ts">
  ...
  let form = $state({
    ...
    user: '',
    database: '',
    tls_mode: 'prefer' as 'disable' | 'prefer' | 'require',
    password: '',
  ...
  <form onsubmit={onSubmit} class="flex flex-col gap-4">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <FormField id="name" label="Name" required error={errors.name}>
        <Input id="name" bind:value={form.name} placeholder="prod-mysql" autofocus />
      </FormField>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <FormField id="host" label="Upstream host" required error={errors.host}>
        <Input id="host" bind:value={form.host} placeholder="mysql.example.com" />
      </FormField>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <FormField id="database" label="Database" hint="Optional default schema.">
        <Input id="database" bind:value={form.database} placeholder="app_db" />
      </FormField>
    </div>

    <FormField id="tls_mode" label="TLS mode (upstream)">
      <select
        id="tls_mode"
